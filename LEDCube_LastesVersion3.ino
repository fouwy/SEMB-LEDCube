...
int SR_DIN = 2;  //cabo roxo
int SR_CLK = 3;  //cabo cinza
...
int impulse_flag = 0, second_impulse = 0;
  ...
  if(rise_flag == 0){
    ...
    for(int i = 1; i < 7; i++){
      if (i <= level){ //se o plano estiver a baixo ou correponder ao nivel atual manter ligado
        image_buffer[7][i][0] = 0b00000000;
        image_buffer[7][i][1] = 0b00000000; 
        image_buffer[7][i][2] = 0b00000000; 
        image_buffer[7][i][3] = 0b00000000; 
        image_buffer[7][i][4] = 0b00000000; 
        image_buffer[7][i][5] = 0b00000000; 
      }
      else{ //se nao esta a acima logo esta desligado
        image_buffer[7][i][0] = 0b11111111;
        image_buffer[7][i][1] = 0b11111111; 
        image_buffer[7][i][2] = 0b11111111; 
        image_buffer[7][i][3] = 0b11111111; 
        image_buffer[7][i][4] = 0b11111111; 
        image_buffer[7][i][5] = 0b11111111; 
      }
    }
...
void setup()
{
  ...
  Sched_AddT(sensorRead, 1, 2);   //reads the sensor
  Sched_AddT(T1, 1, 0);   //one-shot- turn all lights on
  Sched_AddT(shiftRegister, 1, 2);   //updates the lights
  Sched_AddT(heartBeatFreqCalc, 1, 30);  //levels mode task
  Sched_AddT(nextFace, 1, 500); //fading heartbeat mode

  //Add aperiodic task
  AddTaskAper(displayModeSel);
...
void loadNextImage(int iPlan)
{
  if (begin_animation == 1) { //Animation for the hear sim
    I2C_write(PortExpLEFT,  OLAT + 0x10, image_buffer[state_animation[state_counter]][iPlan - 1][0]);
    I2C_write(PortExpLEFT,  OLAT     , image_buffer[state_animation[state_counter]][iPlan - 1][1]);
    I2C_write(PortExpCENTER, OLAT + 0x10, image_buffer[state_animation[state_counter]][iPlan - 1][2]);
    I2C_write(PortExpCENTER, OLAT     , image_buffer[state_animation[state_counter]][iPlan - 1][3]);
    I2C_write(PortExpRIGHT, OLAT + 0x10, image_buffer[state_animation[state_counter]][iPlan - 1][4]);
    I2C_write(PortExpRIGHT, OLAT     , image_buffer[state_animation[state_counter]][iPlan - 1][5]);
    ...
    I2C_write(PortExpLEFT,  OLAT + 0x10, image_buffer[7][iPlan - 1][0]);
    I2C_write(PortExpLEFT,  OLAT     , image_buffer[7][iPlan - 1][1]);
    I2C_write(PortExpCENTER, OLAT + 0x10, image_buffer[7][iPlan - 1][2]);
    I2C_write(PortExpCENTER, OLAT     , image_buffer[7][iPlan - 1][3]);
    I2C_write(PortExpRIGHT, OLAT + 0x10, image_buffer[7][iPlan - 1][4]);
    I2C_write(PortExpRIGHT, OLAT     , image_buffer[7][iPlan - 1][5]);

  }
  else {  //idle stage
    I2C_write(PortExpLEFT,  OLAT + 0x10, image_buffer[mode][iPlan - 1][0]);
    I2C_write(PortExpLEFT,  OLAT     , image_buffer[mode][iPlan - 1][1]);
    I2C_write(PortExpCENTER, OLAT + 0x10, image_buffer[mode][iPlan - 1][2]);
    I2C_write(PortExpCENTER, OLAT     , image_buffer[mode][iPlan - 1][3]);
    I2C_write(PortExpRIGHT, OLAT + 0x10, image_buffer[mode][iPlan - 1][4]);
    I2C_write(PortExpRIGHT, OLAT     , image_buffer[mode][iPlan - 1][5]);
  }
}
...
void nextFace() {
  int8_t next_face;
  //Serial.println(impulse_flag);
  for (int j = 5; j > 0; j--) {
    for (int i = 0; i < 6; i++) {
  
      next_face = image_buffer[8][i][j-1];
      image_buffer[8][i][j] = next_face;
    }
  }
  
  if (impulse_flag == 1) {
    image_buffer[8][0][0] = 0b11111111;
    image_buffer[8][1][0] = 0b00000000;
    image_buffer[8][2][0] = 0b00000000;
    image_buffer[8][3][0] = 0b00000000;
    image_buffer[8][4][0] = 0b11111111;
    image_buffer[8][5][0] = 0b11111111;
    
    impulse_flag = 0;
    second_impulse = 1;
  } else {
    if (second_impulse) {
      image_buffer[8][0][0] = 0b00000000;
      image_buffer[8][1][0] = 0b00000000;
      image_buffer[8][2][0] = 0b00000000;
      image_buffer[8][3][0] = 0b00000000;
      image_buffer[8][4][0] = 0b00000000;
      image_buffer[8][5][0] = 0b11111111;
    
      second_impulse = 0;
    } else {
      image_buffer[8][0][0] = 0b11111111;
      image_buffer[8][1][0] = 0b11111111;
      image_buffer[8][2][0] = 0b00000000;
      image_buffer[8][3][0] = 0b11111111;
      image_buffer[8][4][0] = 0b11111111;
      image_buffer[8][5][0] = 0b11111111;
    }
  }
